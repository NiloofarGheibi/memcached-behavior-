...
def prepare_exp(SSHHost, SSHPort, REMOTEROOT, optpt):
   ...
   #f.write("ssh -F config benchmark \"nohup ls dummy -p 11222 -P memcached.pid > memcached.out 2> memcached.err &\"\n") # adjust this line to properly start memcached
    ...
    f.write("cp -i /root/.ssh/id_rsa id_rsa\n")
    ...
    f.write("RESULT=`ssh -p 10022 server \"pidof memcached\"`\n")
    ...
    #f.write("mcperf --timeout=5 --conn-rate=%d --call-rate=1000 --num-calls=6 --num-conns=50 --sizes=u1,16 -s %s 2>outputest_%d.txt\n" % (optpt["rate"], SSHHost, optpt["rate"]))
    ...
    f.write("ssh -F config benchmark \"kill -9 $(cat /mm/memcached.pid)\"\n")
